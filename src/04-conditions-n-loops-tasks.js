...
function findFirstSingleChar(str) {
  ...
  const firstNonRepeatedChar = str
    .split('')
    .find((char) => charCount.get(char) === 1);
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (openBrackets.includes(item)) {
      ...
      if (
        !lastOpenBracket
        || openBrackets.indexOf(lastOpenBracket) !== closeBrackets.indexOf(item)
      ) {
        return false;
      }
    }
...
function getCommonDirectoryPath(pathes) {
  ...
  const slash = substring.lastIndexOf('/');
  return substring.slice(0, slash + 1);
}
